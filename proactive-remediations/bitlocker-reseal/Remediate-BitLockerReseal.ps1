...
#
# What it does:
#   1. Idempotency: checks sentinel first, exits 0 if already done
#   2. Verifies BitLocker is active on C:
#   3. Runs Suspend-BitLocker -MountPoint C: -RebootCount 1
#      (suspends for exactly ONE reboot, then auto-resumes)
#   4. Writes sentinel: HKLM:\SOFTWARE\CIT\BitLockerReseal\ResealApplied = 1
#   5. The user reboots, BitLocker re-seals to current PCR values, done
#
...
try {
    ...
    # 2. Verify BitLocker is active on C:
    ...
    if ($bl.ProtectionStatus -ne 'On') {
        Write-CITLog -Message "BitLocker protection is $($bl.ProtectionStatus) on C: - not On, skipping re-seal" -Level WARN -ScriptName 'Remediate-BitLockerReseal'
